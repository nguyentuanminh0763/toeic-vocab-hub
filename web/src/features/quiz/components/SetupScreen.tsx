...
export default function SetupScreen({ set = DEFAULT_SET, onStart }: Props) {
  ...
  const maxCount   = mode === 'hard' ? hardCount : totalCount;

  useEffect(() => {
    setHardCount(loadStudyState(set)?.hardSet.length ?? 0);
  }, [set]);
    ...
    <div className="w-full max-w-md md:max-w-xl bg-white rounded-2xl shadow-md p-8 md:p-10 flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="grid grid-cols-2 gap-2">
          {([['all', `Tất cả ${totalCount} từ`], ['hard', `Từ khó (${hardCount})`]] as const).map(([m, label]) => (
            <button
              key={m}
              onClick={() => { setMode(m); setCount(Math.min(count, m === 'hard' ? hardCount : totalCount)); }}
